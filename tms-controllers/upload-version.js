(function(app) {
	...
	app.UploadVersionComponent.prototype.createVersion=function(){
		...
		if(this.name==null || this.name==undefined || this.name==""  || this.name=="null"){
			this.mensaje=null
			this.mensaje="Debe ingresar un nombre identificativo de la versión";
			this.msg.warning();
			//this.resetValuesSendVersion()
			return;
		}
		if(this.version==null || this.version==undefined || this.version==""  || this.version=="null"){
			this.mensaje=null
			this.mensaje="Debe ingresar el número la versión";
			this.msg.warning();
			//this.resetValuesSendVersion()
			return;
		}else{
			...
			if(!utils_Version(this.version)){
				this.mensaje=nullthis.msg.unsubscribe()
				this.mensaje="El formato de la versión de la aplicación es incorrecto, debe tener el siguiente formato 1.X.X";
				this.msg.warning();
				//this.resetValuesSendVersion()
				return;
			}
		...
		if(this.type==null || this.type==undefined || this.type==""  || this.type=="null"){
			this.mensaje=null
			this.mensaje="Debe ingresar el tipo de aplicación";
			this.msg.warning();
			//this.resetValuesSendVersion()
			return;
		}
	...
	app.UploadVersionComponent.prototype.uploadForm=function(file, id, app_id){
		let mensajeAll=null
		mensajeAll = "Se creo la versión pero ocurrio un error al subir el archivo";
		if (file != undefined) {
				...
				reader.onload = async e => {
					...
					for(var i = 0; i < arr.length; i++){
						...
						continueF = await new Promise(async (resolve) => {
							...
							url =getApi()+getEnlaceApi2()+'/tms_pos_app_version/file_chunk?realm='+this.service.getRealm()+'&user_id='+this.service.getUserId()+'&user_email='+this.service.getEmail()+'&business_id='+this.service.getBusinessId()+query	
							
							var contexto=this;	

							const send = await fetch(url, {method:'POST',headers:{'Content-type':'application/octet-stream','Authorization': 'bearer '+this.service.getAccessToken()},body:files}).catch(err => err)
							
							
							if(send){
								if(send.status == 200){
									app.UploadVersionComponent.prototype.progressBarPercent(offset2, current+offset2, size, element)
									offset2+=current
									let i = 0
									if(element.style.width=='100%'){
											let messageInfo = $("#messageInfo")
											let message = $("#messageInfo-msg")
											message.text('exito al subir al version de la app!')
						...
						if(!continueF){
							...
							setTimeout(()=> {
								contexto.mensaje=null
								contexto.mensaje="Ocurrio un error mientras se mandaba el archivo!";
								contexto.msg.error();
							}, 600)
							setTimeout(()=>{
	...
	app.UploadVersionComponent.prototype.progressBarPercent=function(offset, current,  fileSize, element){
		...
		//let bool = false
		let max = fileSize
		percent=((current*100)/max).toFixed(0)
		if(percent == 100){
			//bool=true
			element.style.transition='400ms ease'
			element.style.opacity='0'
